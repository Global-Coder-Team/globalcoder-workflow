...
```json
{
  "permissions": {
    "allow": [
      "Skill(globalcoder-development:using-globalcoder-development)",
      "Skill(globalcoder-development:brainstorming)",
      "Skill(globalcoder-development:ui-design-bootstrap)",
      "Skill(globalcoder-development:writing-plans)",
      "Skill(globalcoder-development:executing-plans)",
      "Skill(globalcoder-development:subagent-driven-development)",
      "Skill(globalcoder-development:test-driven-development)",
      "Skill(globalcoder-development:systematic-debugging)",
      "Skill(globalcoder-development:verification-before-completion)",
      "Skill(globalcoder-development:requesting-code-review)",
      "Skill(globalcoder-development:finishing-a-development-branch)",
      "Task(code-reviewer)"
    ]
  }
}
...
For any of the other skills listed in the [All Skills](#all-skills) section below, use the same `Skill(globalcoder-development:<name>)` pattern. Slash commands (`/brainstorm`, `/write-plan`, `/execute-plan`) inherit permissions from the skills they invoke — no separate command permission needed.
...
| `using-globalcoder-development` | Auto-loaded on session start — establishes how to find and use skills |
...
- security-review
- using-globalcoder-development

---

## Hooks

The plugin includes a `SessionStart` hook that runs on startup, resume, clear, and compact. It automatically injects the `using-globalcoder-development` skill content into the session, establishing skill discovery behavior.
...
- **Cross-references:** Use `globalcoder-development:<skill-name>` format
...
After making changes to `~/Projects/globalcoder-development/`:

```bash
# Tag a new version
cd ~/Projects/globalcoder-development
...
The plugin cache lives at `~/.claude/plugins/cache/globalcoder-marketplace/globalcoder-development/<version>/`.
