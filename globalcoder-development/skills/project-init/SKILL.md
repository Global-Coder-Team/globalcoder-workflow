...
Next: for the first feature, invoke globalcoder-development:brainstorming.
```
...
This project uses the **globalcoder-development** plugin. All creative work follows this sequence, in order:

1. **Brainstorm** (`globalcoder-development:brainstorming`) — Explore intent, requirements, alternatives. Produce a design doc under `docs/plans/`.
2. **UI Design Bootstrap** (`globalcoder-development:ui-design-bootstrap`) — Only if the design has UI. Codify visual decisions into `DESIGN.md` before any component code.
3. **Write Plan** (`globalcoder-development:writing-plans`) — Convert the design into bite-sized TDD tasks.
4. **Execute** — Choose one of `subagent-driven-development`, `agent-team-development`, or `tmux-parallel-development`.
5. **Verify** (`globalcoder-development:verification-before-completion`) — Confirm tests pass and build succeeds.
6. **Finish** (`globalcoder-development:finishing-a-development-branch`) — Merge, PR, or cleanup.
...
- **DESIGN.md** *(if this project has UI)* — Design-token contract (colors, typography, spacing, components). Created by `globalcoder-development:ui-design-bootstrap` before any component code is written.
...
- **After this skill:** First feature work → `globalcoder-development:brainstorming`. The CLAUDE.md template you wrote enforces this.
- **Pairs with:** `globalcoder-development:using-git-worktrees` for isolating feature work once development begins.
- **Do not call:** `globalcoder-development:writing-plans` before `brainstorming` has produced a design doc. CLAUDE.md enforces this order.
